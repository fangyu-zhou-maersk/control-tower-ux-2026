...
Use this process after every meaningful visual implementation pass. The goal is to replace subjective approval with a repeatable comparison between the approved reference and the rendered implementation.

## Required inputs

- approved reference image or Figma frame
- rendered implementation screenshot
- identical viewport width and height
- identical application state and representative data
- completed `SCREEN_SPEC.md`
...
5. Record discrepancies in the table below.
...
- [ ] Fixed, sticky, and scrolling regions behave correctly.
...
- [ ] Outer padding and internal padding match.
- [ ] No default browser or unintended component margin remains.
...
- [ ] Correct MDS or existing project component is used.
...
- [ ] Loading, empty, disabled, error, and success states match the specification.
- [ ] Accessibility is not sacrificed for visual matching.

### Responsive behavior

- [ ] Required breakpoints or container states have been tested.
- [ ] Content reflows according to `SCREEN_SPEC.md`.
- [ ] No unintended overflow, clipping, or overlap occurs.
- [ ] Priority content remains available.

## Agent correction prompt

Use the following instruction after attaching both the reference and implementation screenshots:

```text
Act as a visual implementation reviewer, not a designer.

Compare the approved reference image with the current rendered implementation at the same viewport. Do not redesign, simplify, modernize, or add features.
...
Then implement only the Blocker and High corrections. Preserve working behavior, use existing project components and MDS APIs, and do not make unrelated changes.

After editing, summarize the exact corrections and list any remaining uncertainties or MDS constraints. Do not claim pixel-perfect completion until a new rendered screenshot has been compared.
```
